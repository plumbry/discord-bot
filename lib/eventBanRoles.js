...
function isEventBanBanType(banType) {
  return Boolean(rolesForBanType(banType)?.eventBan);
}

async function removeRolesForBanType(guild, discordId, banType, options = {}) {
  const want = rolesForBanType(banType);
  ...
  if (
    ...
    return {
      ...
      changed: false,
      noop: true,
      reason: "preserve_event_ban_role"
    };
  }
...
async function memberHasEventBanRole(guild, userId) {
  if (!guild || !userId) {
    return false;
  }
  ...
  return Boolean(member?.roles?.cache?.has(EVENT_BAN_ROLE_ID));
}
