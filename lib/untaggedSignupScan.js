...
function extractUsernameTokens(content) {
  if (!content?.trim()) {
    return [];
  }
...
async function resolveUsernameTokens(guild, tokens, sessionCache) {
  ...
  for (const token of tokens) {
    ...
    if (!resolved.ok) {
      unresolved.push({
        token,
        ...resolved,
        slot: resolved.slot ?? token
      });
      continue;
    }

    if (userIds.has(resolved.member.user.id)) {
      continue;
    }

    userIds.add(resolved.member.user.id);
    resolvedUsers.push(resolved.member.user);
  }

  return { resolvedUsers, unresolved };
}

async function collectUsersFromUntaggedMessages(
  messages,
  guild,
  sessionCache
) {
  ...
  for (const message of messages) {
    if (message.author?.bot || messageHasMentions(message)) {
      continue;
    }

    allTokens.push(...extractUsernameTokens(message.content));
  }

  return resolveUsernameTokens(guild, allTokens, sessionCache);
}
