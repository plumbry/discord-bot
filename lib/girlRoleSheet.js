...
async function ensureGirlCacheReady() {
  if (!girlCacheReady && isConfigured()) {
    await loadGirlCache();
  }
}

/** Whether this Discord ID is on the Girl Role sheet (verified female). */
async function isGirlVerifiedOnSheet(discordId) {
  if (!isConfigured()) {
    return false;
  }
  ...
  await ensureGirlCacheReady();

  return girlCache.has(id);
}
...
module.exports = {
  ...
  isConfigured,
  loadGirlCache,
  ensureGirlCacheReady,
  isGirlVerifiedOnSheet,
  addGirlVerified,
