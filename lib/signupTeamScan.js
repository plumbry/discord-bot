...
function messageHasValidDuoSoloPairingSignup(message) {
  return (
    messageHasExactTaggedPlayers(message, 1) ||
    messageHasExactTaggedPlayers(message, 2)
  );
}

function applyDuoSoloPairingFilter(validTeams, maxMatches) {
  const duos = [];
  const solos = [];
  ...
  const acceptedDuos = new Set(
    duos
      .slice(0, pairCount)
      .map(team => getSignupTeamKey(team))
  );
  const acceptedSolos = new Set(
    solos
      .slice(0, pairCount)
      .map(team => getSignupTeamKey(team))
  );

  return validTeams.filter(team => {
    const teamKey = getSignupTeamKey(team);

    return (
      acceptedDuos.has(teamKey) ||
      acceptedSolos.has(teamKey)
    );
  });
}
...
async function syncInvalidSignupReactions(
  messages,
  requiredTeamSize,
  { duoSoloPairing = false } = {}
) {
  let synced = 0;
  ...
  for (const message of messages) {
    ...
    if (duoSoloPairing) {
      if (messageHasValidDuoSoloPairingSignup(message)) {
        continue;
      }
    } else if (
      messageHasExactTaggedPlayers(message, requiredTeamSize)
    ) {
      continue;
    }
