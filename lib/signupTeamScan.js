...
/** One-off Chili-sponsored 24-team Reload (BBL chili_doug). */
const SPONSORED_RELOAD_USER_ID = "868996533532622928";
const SPONSORED_RELOAD_TEAM_LIMIT = 24;

function teamIncludesUserId(team, userId) {
  return team.users.some(user => user.id === userId);
}

function splitSponsoredReloadTeams(
  validTeams,
  {
    sponsorUserId = SPONSORED_RELOAD_USER_ID,
    teamLimit = SPONSORED_RELOAD_TEAM_LIMIT
  } = {}
) {
  const reservedIndex = validTeams.findIndex(team =>
    teamIncludesUserId(team, sponsorUserId)
  );

  if (reservedIndex === -1 || reservedIndex < teamLimit) {
    return {
  ...
  const reservedNonSponsorSlots = teamLimit - 1;
  const roledTeams = [];
  const overflowTeams = [];
  let nonSponsorPlaying = 0;

  for (const team of validTeams) {
    if (teamIncludesUserId(team, sponsorUserId)) {
      roledTeams.push(team);
      continue;
    }

    if (nonSponsorPlaying < reservedNonSponsorSlots) {
      roledTeams.push(team);
      nonSponsorPlaying++;
      continue;
    }
...
function splitValidTeams(
  validTeams,
  {
    ...
    sponsoredReload = false,
    sponsorUserId = SPONSORED_RELOAD_USER_ID
  }
) {
  if (sponsoredReload) {
    return splitSponsoredReloadTeams(validTeams, {
      sponsorUserId,
      teamLimit: teamLimitOverride ?? SPONSORED_RELOAD_TEAM_LIMIT
    });
  }
...
async function scanSignupTeams(channel, guild, {
  ...
  for (const team of candidateTeams) {
    ...
    for (const user of team.users) {
      ...
      if (!hasEventBanRole) {
        console.warn(
          "[SIGNUP SCAN] Ignoring active event-ban sheet row because member lacks role:",
          user.id
        );
        continue;
      }
