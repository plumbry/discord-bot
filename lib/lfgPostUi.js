...
function publicPostContent(eventName, startAt) {
  return [
...
function needFlowRows(eventId, selectedTiers = [], selectedGender = "") {
  return [
    new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
    ...
    new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .setPlaceholder("Required gender")
        .addOptions(
          GENDER_OPTIONS.map(option => ({
            label: option.label,
            value: option.value,
            default: selectedGender === option.value
          }))
        )
    ),
    new ActionRowBuilder().addComponents(
...
function formatNeedSummary(acceptedTiers, requiredGender) {
  ...
  const gender =
    requiredGender === "either"
      ? "Either"
      : POST_GENDER_LABEL[requiredGender] || requiredGender;
