...
function fillMatchDm({ tier, gender, displayName, username }) {
  const genderLabel = POST_GENDER_LABEL[gender] || gender;
  const name = displayName || username || "a player";
  const handle =
    username && displayName && displayName.toLowerCase() !== username.toLowerCase()
      ? ` (@${username})`
      : "";

  return [
    `**${tier} Tier ${genderLabel} ${name}${handle} is looking to fill!**`,
    "",
