...
const ACTIONS = [
  ...
  },
  {
    key: "voicecheck",
    label: "Voice Check",
    fixedChannelId: "1471082166535454780",
    permissions: [
      PermissionFlagsBits.ViewChannel,
      PermissionFlagsBits.SendMessages
    ]
...
function resolveActionChannel({
  ...
  if (action.fixedChannelId || action.useDashboardChannel) {
    const channel = action.fixedChannelId
      ? guild.channels.cache.get(action.fixedChannelId)
      : dashboardChannel || guild.channels.cache.get(
        DEFAULT_DASHBOARD_CHANNEL_ID
      );
    ...
    if (!channel?.isTextBased?.()) {
      return {
        ...
        channelId: "",
        matches: [],
        warning: action.fixedChannelId
          ? "Configured fixed channel was not found."
          : "Dashboard channel was not found."
      };
    }
  ...
  if (matches.length > 1 && !action.allowMultiple) {
    return {
      key: action.key,
      label: action.label,
      status: "ambiguous",
  ...
  const missingByChannel = matches
    .map(channel => ({
      channel,
      missing: getMissingPermissions(channel, action.permissions)
    }))
  ...
  return {
    ...
    matches: matches.map(channel => channel.id),
    warning: missingByChannel.length
      ? missingByChannel
        .map(entry =>
          `${entry.channel}: ${formatPermissionWarning(entry.missing)}`
        )
        .join("; ")
      : null
  };
