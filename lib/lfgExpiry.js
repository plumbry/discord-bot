...
const { closedPostContent } = require("./lfgPostUi");
...
async function disableLfgPostMessage(client, config) {
  if (!client || !config?.lfgChannelId || !config?.lfgMessageId) {
    return;
  }

  try {
    const channel = await client.channels.fetch(config.lfgChannelId);
    const message = await channel?.messages
      ?.fetch(config.lfgMessageId)
      .catch(() => null);
    ...
    await message.edit({
      content: closedPostContent(config.eventName, config.startTime),
      components: []
    });
  } catch (err) {
    console.warn(
      `[LFG] failed to close LFG post message for ${config.eventName}:`,
      err?.message || err
    );
  }
}
...
async function expireLfgEvent(client, config, reason = "event_started") {
  if (config.lfgEnabled) {
    await upsertLfgEvent({
      ...
      lfgEnabled: false,
      lfgPostEnabled: false
    });
    config.lfgEnabled = false;
    config.lfgPostEnabled = false;
  }

  await disableLfgPostMessage(client, config);
...
async function endLfgPost(client, config, reason = "ended") {
  await upsertLfgEvent({
    discordEventId: config.discordEventId,
    guildId: config.guildId,
    lfgPostEnabled: false
  ...
  const closed = await closePostRequestsForEvent(
    config.discordEventId,
    reason
  );
  ...
  for (const request of closed) {
    if (!isPostNeed(request) || notified.has(request.ownerUserId)) {
      continue;
    }

    notified.add(request.ownerUserId);

    try {
      const user = await client.users.fetch(request.ownerUserId);
      await user.send(
        `The LFG post for **${config.eventName}** has ended. You won't get more fill DMs for this event.`
      );
    } catch (err) {
      console.warn(
        `[LFGPOST] end DM failed for ${request.ownerUserId}:`,
        err?.message || err
      );
    }
  }
  ...
  if (closed.length) {
    console.log(
      `[LFGPOST] ended post for ${config.eventName}; closed ${closed.length} fill/need record(s)`
    );
  }
