...
function normalizeTwitchLogin(segment) {
  const lower = String(segment || "").toLowerCase();

  return TWITCH_LOGIN_REGEX.test(lower) ? lower : null;
}

function parseAllTwitchFromContent(content) {
  const logins = [];
  const invalidLogins = [];
  const videoIds = [];
  ...
  for (const match of content.matchAll(/twitch\.tv\/([^/?#\s]+)/gi)) {
    let segment = match[1];

    const videoMatch = segment.match(/^videos\/(\d+)$/i);
    if (videoMatch) {
    ...
    if (RESERVED_TWITCH_PATHS.has(raw)) {
      continue;
    }
    ...
    if (seenInvalid.has(raw)) continue;
    seenInvalid.add(raw);
    invalidLogins.push(raw);
  }

  return { logins, invalidLogins, videoIds };
}
...
async function collectPostedChannelEntries(streamChannel, token) {
  ...
  for (const msg of messages) {
    ...
    const { logins, invalidLogins, videoIds } = parseAllTwitchFromContent(msg.content);
    if (!logins.length && !invalidLogins.length && !videoIds.length) continue;

    for (const videoId of videoIds) {
      const vod = await getVideoById(videoId, token);
      const resolvedLogin = vod?.user_login?.toLowerCase() || null;
...
async function collectPostedChannels(streamChannel, token) {
  ...
  for (const { login, videoId, invalid } of entries) {
    if (seen.has(login)) continue;

    seen.add(login);
    channels.push({ login, videoId, invalid: Boolean(invalid) });
  }
...
function parseTeamNumberFromContent(content) {
  const teamMatch = content.match(/\bteam\s*#?\s*(\d+)\b/i);
  if (teamMatch) {
...
async function resolveLoginsFromContent(content, token) {
  const { logins, videoIds } = parseAllTwitchFromContent(content);
  const resolved = [...logins];

  for (const videoId of videoIds) {
    const vod = await getVideoById(videoId, token);
    const login = normalizeTwitchLogin(vod?.user_login);

    if (login && !resolved.includes(login)) {
      resolved.push(login);
    }
  }
...
async function collectTeamTaggedLogins(streamChannel, token) {
  const messages = await fetchAllMessages(streamChannel);
  const byTeam = new Map();

  for (const msg of messages) {
    if (msg.author.bot || !isStaffPoster(msg)) continue;

    const teamNumber = parseTeamNumberFromContent(msg.content);
    if (!teamNumber) continue;

    const logins = await resolveLoginsFromContent(msg.content, token);
    if (!logins.length) continue;

    if (!byTeam.has(teamNumber)) {
...
async function scanLoginVods({ logins, token, start, end }) {
  ...
  for (const { login, videoId, invalid } of logins) {
    if (invalid) {
      results.push({
        twitch: login,
        valid: false,
