...
function normalizeTwitchLogin(segment) {
  ...
  if (TWITCH_LOGIN_REGEX.test(lower)) {
    return lower;
  }

  const ascii = lower.replace(/[^a-z0-9_]/g, "");

  if (TWITCH_LOGIN_REGEX.test(ascii)) {
    return ascii;
  }
...
function parseAllTwitchFromContent(content) {
  ...
  for (const match of content.matchAll(/twitch\.tv\/([^/?#\s]+)/gi)) {
    ...
    if (RESERVED_TWITCH_PATHS.has(segment.toLowerCase())) {
      continue;
    }

    const login = normalizeTwitchLogin(segment);
...
async function collectPostedChannelEntries(streamChannel, token) {
  ...
  for (const msg of messages) {
    ...
    const { logins, videoIds } = parseAllTwitchFromContent(msg.content);
    if (!logins.length && !videoIds.length) continue;
