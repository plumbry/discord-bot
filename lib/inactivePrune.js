...
} = require("./websiteYunitePlayed");
const { loadRecentMessageUserIds } = require("./recentDiscordMessages");
...
async function getInteractionRecord(member, activityIds, recentMessages, minAgeDays) {
  const lastMessageAt = recentMessages?.lastByUser?.get(member.id);

  if (lastMessageAt || recentMessages?.ids?.has(member.id)) {
    return {
      found: true,
      reason: lastMessageAt
        ? `Sent a Discord message on ${formatJoinedDate(lastMessageAt)}`
        : `Sent a Discord message in the last ${minAgeDays} day(s)`
    };
  }
...
async function classifyMember(member, options) {
  ...
  const interaction = await getInteractionRecord(
    member,
    activityIds,
    recentMessages,
    minAgeDays
  ...
  if (recentMessages && !recentMessages.complete) {
    record.bucket = "unknown";
    record.eligibilityReason =
      "Recent Discord messages could not be scanned";
    return record;
  }
  ...
  record.eligibilityReason =
    `Joined ${ageDays} days ago, no stored interaction, no recent Discord messages, never played a Yunite scrim`;
  return record;
}
...
async function scanGuildMembers(guild, options = {}) {
  ...
  const [activity, snapshot, recentMessages] = await Promise.all([
    loadStoredActivityUserIds(),
    loadYuniteSnapshot(discordIds),
    loadRecentMessageUserIds(guild, { minAgeDays, now })
  ]);
  ...
  return {
    ...
    recentMessages: {
      ...
      channelCount: recentMessages.channelCount,
      scannedChannels: recentMessages.scannedChannels,
      failedChannels: recentMessages.failedChannels,
      recentPosterCount: recentMessages.ids.size
    },
    yunite: {
