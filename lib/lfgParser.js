...
function resolveWhenBucket(targetWhenMs, referenceNow = new Date()) {
  ...
  const dayAfterTomorrow = today + 2 * MS_PER_DAY;
  ...
  if (targetDay === tomorrow) {
    return { label: "Tomorrow", whenSortKey: 1 };
  }

  if (targetDay === dayAfterTomorrow) {
    const weekday = new Date(dayAfterTomorrow + 12 * 60 * 60 * 1000).getUTCDay();
    const weekdayName = WEEKDAYS[weekday];
    const label =
      weekdayName.charAt(0).toUpperCase() + weekdayName.slice(1);
    return { label, whenSortKey: 2 };
  }
...
function parseWhenLabel(
  ...
  const lower = text.toLowerCase();
  const anchor = new Date(messagePostedAt);

  if (TOMORROW_WHEN.test(lower)) {
    return {
      label: "Tomorrow",
      sortKey: addLogicalDays(anchor, 1)
    };
  }
  ...
  if (TODAY_WHEN.test(lower) || FOR_TODAY.test(lower)) {
    return {
      label: "Today / Tonight",
      sortKey: startOfLogicalDayUtc(anchor)
    };
  }
  ...
  for (let i = 0; i < WEEKDAYS.length; i++) {
    ...
    const current = logicalWeekdayUtc(anchor);
    let delta = i - current;
    ...
    return {
      label,
      sortKey: startOfLogicalDayUtc(anchor) + delta * MS_PER_DAY
    };
  }
