...
/** Event-driven sync from the website (no Convex member lookup). */
async function syncFemaleEvaluatedRoleFromPush(
  guild,
  {
    discordId,
    evaluatedGender,
  ...
  if (!id) {
    return { ok: false, action: "none", reason: "missing_discord_id" };
  }

  if (typeof evaluatedGender !== "number" || !Number.isFinite(evaluatedGender)) {
    return { ok: false, action: "none", reason: "invalid_gender" };
  }

  const member = await resolveGuildMember(guild, id);

  if (!member || member.user.bot) {
    return { ok: true, action: "none", reason: "not_in_guild" };
  }

  const context = [source, eventType && `event=${eventType}`, memberId && `member=${memberId}`]
    .filter(Boolean)
    .join(" ");

  if (evaluatedGender === FEMALE_GENDER_VALUE) {
    return addFemalePendingRoleIfEligible(member, { source: context });
  }

  return removeFemalePendingRoleIfPresent(member, { source: context });
}
...
module.exports = {
  ...
  syncFemaleEvaluatedRoleFromPush
};
